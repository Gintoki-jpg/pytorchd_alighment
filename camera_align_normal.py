...
def main():
    ...
    cameras = FoVPerspectiveCameras(znear=0.1,zfar=10000,aspect_ratio=0.836,fov=30,degrees=True,device=device)      # 使用透视相机（遵循blender中的设定）

    blend_params = BlendParams(sigma=1e-4, gamma=1e-4)  # 默认渲染参数

    silhouette_renderer = create_renderer('NormalShader',image_size=image_size,blend_params=blend_params,cameras=cameras,device=device)
